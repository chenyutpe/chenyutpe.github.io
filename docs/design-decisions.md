...
- Theme-dependent text colors transition over 500 ms with `ease`, owned by each text block (`theme-text` or an existing component rule). `theme-text` defaults to foreground; color utilities can override it. Do not animate `color` on the body or shared containers above independently animated text: browser traces showed child transitions restarting each frame and taking about a second to finish. For mixed-color sentences, animate sibling spans, not both the paragraph and its colored children. Plain descendants can inherit their text block's animated color. Keep background, hover, entrance, and reduced-motion behavior intact.
